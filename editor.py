...
class MusicLayoutEditor(tk.Tk):
    """UI for manually editing layout boundaries and metadata."""
    def __init__(self, pdf_path, layout_path):
        ...
        if not (0 <= self.current_page_idx < len(self.pages)): 
            self.current_page_idx = 0
        
        # Pull DPI from Layout fallback to 100
        self.dpi = self.layout.get("DPI", 100)
    ...
    def save_state(self):
        """Pushes current state to undo stack, wiping redo stack."""
    ...
    def draw_boxes(self):
        ...
        for i, sys in enumerate(self.page_data.get("systems", [])):
            ...
            # Highlight border if this is the hovered index
            if getattr(self, 'hovered_idx', None) == i: width += 2
    ...
    def on_mouse_move(self, event):
        ...
        new_hover = self.get_system_at_y(y)
        if new_hover != getattr(self, 'hovered_idx', None):
            self.hovered_idx = new_hover
            self.draw_boxes()

    def on_delete_key(self, event):
        idx = getattr(self, 'hovered_idx', None)
        if idx is not None:
            self.save_state()
    ...
    def on_release(self, event):
        if self.drag_data.get("item_idx") == "new":
            ...
            if abs(y1 - y0) > 5:  
                new_sys = {"top": min(y0, y1) / self.scale, "bot": max(y0, y1) / self.scale, "is_content": True, "is_reset": False}
    ...
    def on_right_click(self, event):
        idx = self.get_system_at_y(self.canvas.canvasy(event.y))
        if idx is not None:
            self.save_state()
